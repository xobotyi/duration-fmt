...
describe('parse', () => {
	...
	test('invalid output unit', () => {
		...
		}).toThrowError('Invalid output unit option, it must be present in the units array');
	});
	...
	describe('custom units', () => {
		...
		test.each<{
			...
			{ args: ['0с', { units }], out: 0 },
			{ args: ['1с', { units }], out: 1 },
			{ args: ['3д', { units }], out: 3 * 86_400 },
			{ args: ['7нед', { units }], out: 7 * 604_800 },
			{ args: ['2д 5с', { units }], out: 2 * 86_400 + 5 },
			{
				args: ['2д 5с', { units, outputUnit: 'д' }],
				out: (2 * 86_400 + 5) / 86_400,
			},
